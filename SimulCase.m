...
% Code by Baolei Wei
% For paper, "On unification framework for continuous-time grey models: 
%             An integral matching perspective"
% by Baolei Wei, Naiming Xie
% https://doi.org/10.1016/j.apm.2021.09.008
...
% fplot([x1Sol x2Sol],[0 5],'Linewidth',2)
...
for iter_h = 1:length(h_set)
    ...
    for iter_snr = 1:length(snr_set)
        ...
        %% mape values for fitting and forecasting 
        gm_name = sprintf('x1_mape_%d_%d_gm.csv', round(snr), 5/h+1); 
        csvwrite(['results/',gm_name], x1_mape_gm)
        gm_name = sprintf('x2_mape_%d_%d_gm.csv', round(snr), 5/h+1); 
        csvwrite(['results/',gm_name], x2_mape_gm)
        
        im_name = sprintf('x1_mape_%d_%d_im.csv', round(snr), 5/h+1); 
        csvwrite(['results/',im_name], x1_mape_im)
        im_name = sprintf('x2_mape_%d_%d_im.csv', round(snr), 5/h+1); 
